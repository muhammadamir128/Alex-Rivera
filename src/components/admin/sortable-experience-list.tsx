...
import { GripVertical, MapPin, CalendarDays, Pencil, Trash2 } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { motion } from "framer-motion";
import { cn } from "@/lib/utils";

export type SortableExperience = {
  id: string;
  role: string;
  company: string;
...
export function SortableExperienceList({
  ...
  items: SortableExperience[];
  onReorder: (newItems: SortableExperience[]) => void;
  onEdit: (item: SortableExperience) => void;
  onDelete: (id: string) => void;
}) {
  const [localItems, setLocalItems] = useState(items);

  // sync with parent when items change structurally (not during drag)
  const sameIds =
